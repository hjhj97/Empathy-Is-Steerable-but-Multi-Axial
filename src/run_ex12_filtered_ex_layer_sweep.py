#!/usr/bin/env python3
"""Ex12: filtered EX dataset layer-sweep steering experiment.

Design summary:
1) Build EX steering vectors with CAA contrast on filtered dataset:
   - positive pool: EX level >= 1
   - negative pool: EX level == 0
...
DEFAULT_DATASET = Path("dataset/filtered/explorations-reddit-filtered.csv")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser("Ex12 filtered EX layer-sweep steering")
    ...
    p.add_argument("--output-dir", type=Path, default=Path("outputs/ex12_filtered_ex"))
    p.add_argument("--run-name", type=str, default="ex12_llama31_filtered_ex_n200")
    return p.parse_args()
...
def summarize_by_layer_alpha(classified_df: pd.DataFrame, baseline_ex: float) -> pd.DataFrame:
    rows: List[dict] = []
    for (layer, alpha), g in classified_df.groupby(["layer", "alpha"], sort=True):
        ...
        rows.append({
            ...
            "n": int(len(g)),
            "EX_mean": mean_ex,
            "EX_pct_0": float((g["EX_label"] == 0).mean()),
            "EX_pct_1": float((g["EX_label"] == 1).mean()),
            "EX_pct_2": float((g["EX_label"] == 2).mean()),
            "delta_vs_unsteered": mean_ex - baseline_ex,
        })
    out = pd.DataFrame(rows)
    if out.empty:
        return out
    alpha0 = out[np.isclose(out["alpha"], 0.0)].set_index("layer")["EX_mean"].to_dict()
    out["delta_vs_alpha0"] = out.apply(
        lambda r: r["EX_mean"] - alpha0.get(int(r["layer"]), float("nan")), axis=1
    )
...
def main() -> None:
    ...
    baseline_ex = float(classified_df[np.isclose(classified_df["alpha"], 0.0)]["EX_label"].mean())
    summary_df = summarize_by_layer_alpha(classified_df, baseline_ex=baseline_ex)
    ...
    metadata = {
        "task": "ex12_filtered_ex_layer_sweep",
        ...
        "baseline_ex_alpha0": baseline_ex,
    ...
    print("\n" + "=" * 60, flush=True)
    print("SUMMARY (EX_mean by layer x alpha)", flush=True)
    print("=" * 60, flush=True)
    print(summary_df.pivot_table(index="layer", columns="alpha", values="EX_mean").to_string(), flush=True)
