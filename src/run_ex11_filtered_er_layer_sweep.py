#!/usr/bin/env python3
"""Ex11: filtered ER dataset layer-sweep steering experiment.

Design summary:
1) Build ER steering vectors with CAA contrast on filtered dataset:
   - positive pool: ER level >= 1
   - negative pool: ER level == 0
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser("Ex11 filtered ER layer-sweep steering")
    ...
    p.add_argument("--output-dir", type=Path, default=Path("outputs/ex11_filtered_er"))
    p.add_argument("--run-name", type=str, default="ex11_llama31_filtered_er_n200")
    return p.parse_args()
...
def summarize_by_layer_alpha(classified_df: pd.DataFrame, baseline_er: float) -> pd.DataFrame:
    rows: List[dict] = []
    for (layer, alpha), g in classified_df.groupby(["layer", "alpha"], sort=True):
        ...
        rows.append({
            ...
            "delta_vs_unsteered": mean_er - baseline_er,
        })
    out = pd.DataFrame(rows)
    if out.empty:
        return out
    alpha0 = out[np.isclose(out["alpha"], 0.0)].set_index("layer")["ER_mean"].to_dict()
    out["delta_vs_alpha0"] = out.apply(
        lambda r: r["ER_mean"] - alpha0.get(int(r["layer"]), float("nan")), axis=1
    )
...
def main() -> None:
    ...
    summary_df = summarize_by_layer_alpha(classified_df, baseline_er=baseline_er)
    ...
    metadata = {
        "task": "ex11_filtered_er_layer_sweep",
    ...
    print("\n" + "=" * 60, flush=True)
    print("SUMMARY (ER_mean by layer x alpha)", flush=True)
    print("=" * 60, flush=True)
    print(summary_df.pivot_table(index="layer", columns="alpha", values="ER_mean").to_string(), flush=True)
