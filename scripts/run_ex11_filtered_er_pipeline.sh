...
PY="${PYTHON:-python}"
SCRIPT="${ROOT}/src/run_ex11_filtered_er_layer_sweep.py"

DATASET_PATH="${DATASET_PATH:-${ROOT}/dataset/filtered/emotional-reactions-reddit-filtered.csv}"
...
RUN_PREFIX="${RUN_PREFIX:-ex11_filtered_er}"
OUT_DIR="${ROOT}/outputs/ex11_filtered_er"
LOG_DIR="${ROOT}/logs"
...
run_one() {
  ...
  local -a cmd=(
    ...
    --output-dir "outputs/ex11_filtered_er"
    --run-name "${run_name}"
  )
...
echo "[done] $(date '+%F %T') Ex11 pipeline finished" | tee -a "${PIPELINE_LOG}"
