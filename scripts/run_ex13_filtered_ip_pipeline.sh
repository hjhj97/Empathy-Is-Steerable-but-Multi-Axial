...
PY="${PYTHON:-python}"
SCRIPT="${ROOT}/src/run_ex13_filtered_ip_layer_sweep.py"

DATASET_PATH="${DATASET_PATH:-${ROOT}/dataset/filtered/interpretations-reddit-filtered.csv}"
...
RUN_PREFIX="${RUN_PREFIX:-ex13_filtered_ip}"
OUT_DIR="${ROOT}/outputs/ex13_filtered_ip"
LOG_DIR="${ROOT}/logs"
...
run_one() {
  ...
  local -a cmd=(
    ...
    --output-dir "outputs/ex13_filtered_ip"
    --run-name "${run_name}"
  )
...
echo "[done] $(date '+%F %T') Ex13 pipeline finished" | tee -a "${PIPELINE_LOG}"
